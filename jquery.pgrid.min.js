...
else s[v+1]=u(c(this).contents().text())})});p=p.replace("__title__",t);for(var w in s)isNaN(w)||(p=p.replace("__col_"+w+"__",s[w]));if(j.button==1||d.target&&d.target!="_self"){if(d.window_features)return window.open(p,d.target,d.window_features);return window.open(p,d.target)}else{if(typeof pines!="undefined"&&pines.get)return pines.get(p);return window.location=p}}return true}).mousedown(function(){return false}).hover(function(){c(this).addClass("ui-state-hover")},function(){c(this).removeClass("ui-state-hover")});
