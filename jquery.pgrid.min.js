...
!d.select_all&&!d.select_none&&h.length>1){alert("Please choose only one item before performing this operation.");return false}if(d.confirm)if(typeof d.confirm=="function"){if(!d.confirm(j,h))return false}else if(d.return_all_rows){if(!confirm('Are you sure you want to perform the operation "'+d.text+'" on all items?'))return false}else if(h.length===0){if(!confirm('Are you sure you want to perform the operation "'+d.text+'"?'))return false}else if(!confirm('Are you sure you want to perform the operation "'+
d.text+'" on the '+h.length+" currently selected item(s)?"))return false;if(d.select_all)if(a.pgrid_select&&a.pgrid_multi_select){a.children("tbody").children("tr:not(.ui-helper-hidden)").addClass("ui-pgrid-table-row-selected ui-state-active");a.update_selected()}if(d.select_none)if(a.pgrid_select){a.children("tbody").children().removeClass("ui-pgrid-table-row-selected").removeClass("ui-state-active");a.update_selected()}if(d.click){var n="";if(d.pass_csv||d.pass_csv_with_headers){if(d.pass_csv_with_headers)h=
a.children("thead").children().add(h);h.each(function(){c(this).children("th:not(.ui-pgrid-table-expander), td:not(.ui-pgrid-table-expander)").each(function(){var r=c(this);n+='"'+r.contents().text().replace('"','""')+'"';if(r.next())n+=","});n+="\n"})}else n=h;return d.click(j,n)}else if(d.url){var p=d.url,t="",s=[];h.each(function(){var r=c(this),y=r.children("td:not(.ui-pgrid-table-expander)");t+=(t.length?d.delimiter:"")+u(r.attr("title"));y.each(function(v){if(s[v+1])s[v+1]+=d.delimiter+u(c(this).contents().text());
else s[v+1]=u(c(this).contents().text())})});p=p.replace("__title__",t);for(var w in s)isNaN(w)||(p=p.replace("__col_"+w+"__",s[w]));if(j.button==1||d.target&&d.target!="_self"){if(d.window_features)return window.open(p,d.target,d.window_features);return window.open(p,d.target)}else{if(typeof pines!="undefined"&&pines.get)return pines.get(p);return window.location=p}}return true});typeof d.title!="undefined"&&g.attr("title",d.title);if(d.double_click){if(a.pgrid_double_click_tb)var i=a.pgrid_double_click_tb;
a.pgrid_double_click_tb=function(){i&&i();g.mouseup()}}a.toolbar.append(g)}else if(d.type=="text"){b=c("<div />").addClass("ui-pgrid-toolbar-text");d.extra_class&&b.addClass(d.extra_class);typeof d.title!="undefined"&&b.attr("title",d.title);d.label&&b.append(c("<span>"+d.label+"</span>").addClass("ui-pgrid-toolbar-text-label"));var k=c('<input type="text" />').addClass("ui-widget ui-widget-content ui-corner-all");d.dont_prevent_default||k.keydown(function(j){j.keyCode==13&&j.preventDefault()});d.attributes&&
...
a.pgrid_sort_ord=a.pgrid_sort_ord=="desc"?"asc":"desc";l=a.pgrid_sortable;a.pgrid_sortable=true;a.do_sort(a.pgrid_sort_col,true);a.pgrid_sortable=l}if(a.pgrid_paginate){a.addClass("ui-pgrid-paginated");a.pgrid_footer&&a.footer.append(c("<div />").addClass("ui-pgrid-footer-pager-container").each(function(){c(this).append("Display ").append(c("<input />").addClass("ui-widget ui-widget-content ui-corner-all ui-pgrid-per-page").attr({type:"text",value:a.pgrid_perpage}).change(function(){var b=c(this);
