...
a.init_children(b.filter("tr.parent:not(.child)"));b.filter("tr.child").addClass("ui-pgrid-table-row-hidden")}};a.init_children=function(b){b.length&&b.each(function(){var d=c(this),g=parseInt(d.children("td:last-child").css("padding-left"));a.init_children(d.siblings("."+d.attr("title")).each(function(){c(this).children("td:not(.ui-pgrid-table-expander)").css("padding-left",g+10+"px")}).filter("tr.parent"))}).children("td.ui-pgrid-table-expander").append(c("<span />").addClass("ui-icon ui-icon-triangle-1-e")).click(function(){var d=
c(this),g=d.parent();if(g.hasClass("ui-pgrid-table-row-expanded")){g.removeClass("ui-pgrid-table-row-expanded");d.children("span.ui-icon").removeClass("ui-icon-triangle-1-s").addClass("ui-icon-triangle-1-e");a.hide_children(g)}else{g.addClass("ui-pgrid-table-row-expanded");d.children("span.ui-icon").removeClass("ui-icon-triangle-1-e").addClass("ui-icon-triangle-1-s");a.show_children(g)}})};a.pgrid_row_hover_effect&&a.children("tbody").delegate("tr","mouseover",function(){c(this).addClass("ui-state-hover")}).delegate("tr",
...
delete l;a.init_rows(a.children("tbody").children());a.do_col_hiding(true);a.pgrid_table_viewport=c('<div class="ui-pgrid-table-viewport ui-widget-content" />');a.pgrid_table_container.appendTo(a.pgrid_table_viewport);a.pgrid_table_viewport.height(a.pgrid_view_height);a.pgrid_view_height!="auto"&&a.pgrid_table_container.css({position:"absolute",top:"0",bottom:"0",left:"0",right:"0"});if(a.pgrid_toolbar){a.toolbar=c("<div />").addClass("ui-pgrid-toolbar");c.each(a.pgrid_toolbar_contents,function(b,
d){if(d.type=="button"){var g=c("<span />").addClass("ui-pgrid-toolbar-button ui-state-default ui-corner-all").append(c("<span><span>"+d.text+"</span></span>").each(function(){d.extra_class&&c(this).addClass(d.extra_class)})).mouseup(function(j){if(j.button==2)return true;var h=d.return_all_rows?a.children("tbody").children("tr:not(.ui-helper-hidden)"):a.children("tbody").children("tr.ui-pgrid-table-row-selected");if(!d.selection_optional&&!d.select_all&&!d.select_none&&h.length===0){alert("Please make a selection before performing this operation.");
return false}if(!d.multi_select&&!d.selection_optional&&!d.select_all&&!d.select_none&&h.length>1){alert("Please choose only one item before performing this operation.");return false}if(d.confirm)if(d.return_all_rows){if(!confirm('Are you sure you want to perform the operation "'+d.text+'" on all items?'))return false}else if(h.length===0){if(!confirm('Are you sure you want to perform the operation "'+d.text+'"?'))return false}else if(!confirm('Are you sure you want to perform the operation "'+d.text+
'" on the '+h.length+" currently selected item(s)?"))return false;if(d.select_all)if(a.pgrid_select&&a.pgrid_multi_select){a.children("tbody").children("tr:not(.ui-helper-hidden)").addClass("ui-pgrid-table-row-selected ui-state-active");a.update_selected()}if(d.select_none)if(a.pgrid_select){a.children("tbody").children().removeClass("ui-pgrid-table-row-selected").removeClass("ui-state-active");a.update_selected()}if(d.click){var n="";if(d.pass_csv||d.pass_csv_with_headers){if(d.pass_csv_with_headers)h=
...
else s[v+1]=u(c(this).contents().text())})});p=p.replace("__title__",t);for(var w in s)isNaN(w)||(p=p.replace("__col_"+w+"__",s[w]));if(j.button==1||d.target&&d.target!="_self"){if(d.window_features)return window.open(p,d.target,d.window_features);return window.open(p,d.target)}else{if(typeof pines=="object"&&pines.get)return pines.get(p);return window.location=p}}return true}).mousedown(function(){return false}).hover(function(){c(this).addClass("ui-state-hover")},function(){c(this).removeClass("ui-state-hover")});
if(d.double_click){if(a.pgrid_double_click_tb)var i=a.pgrid_double_click_tb;a.pgrid_double_click_tb=function(){i&&i();g.mouseup()}}a.toolbar.append(g)}else if(d.type=="text"){b=c("<span />").addClass("ui-pgrid-toolbar-text");d.extra_class&&b.addClass(d.extra_class);d.label&&b.append(c("<span>"+d.label+"</span>").addClass("ui-pgrid-toolbar-text-label"));var k=c('<input type="text" />').addClass("ui-widget ui-widget-content ui-corner-all");d.dont_prevent_default||k.keydown(function(j){j.keyCode==13&&
j.preventDefault()});d.attributes&&k.attr(d.attributes);b.append(k);a.toolbar.append(b);d.load&&d.load(k)}else d.type=="separator"&&a.toolbar.append(c("<span />").addClass("ui-pgrid-toolbar-sep ui-state-default"));a.toolbar.append('<span class="ui-pgrid-toolbar-spacer"> </span>')})}if(a.pgrid_footer){a.footer=c("<div />").addClass("ui-pgrid-footer ui-widget-header ui-corner-bottom ui-helper-clearfix").delegate("button","mouseenter",function(){c(this).addClass("ui-state-hover")}).delegate("button",
